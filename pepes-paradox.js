(() => {
  ...
  function collect(item,px,py,r,key){const distance=Math.hypot(px-item.x,py-item.y),baseRadius=r+42;if(item.got||distance>baseRadius+state.petCollectRadius)return;if(state.petCollectRadius>0&&distance>baseRadius)window.ArcadePet?.showAssist("paradox",true);item.got=true;player[key]++;burst(item.x,item.y,key==="relics"?"#ffd86c":"#72ff9b",18);tone(key==="relics"?760:620,.1,"sine");syncHud();}
